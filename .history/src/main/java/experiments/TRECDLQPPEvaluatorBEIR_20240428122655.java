...
public class TRECDLQPPEvaluatorBEIR {
    ...
    public static void main(String[] args) {
        ...
        if (args.length < 6) {
            System.out.println("Required arguments: <res file DL 19> <res file DL 20> <metric (ap/ndcg)> <uef/nqc> <corpus>");
            args = new String[7];
        ...
        String corpusName = Constants.MSMARCO_INDEX;
        switch (args[6]) {
            case "covid":
              corpusName = Constants.COVID_INDEX;
              break;
            case "fever":
              corpusName = Constants.FEVER_INDEX;
              break;
            case "touche":
              corpusName = Constants.WEBIS_INDEX;
              break;
        }
        ...
        try {
            OneStepRetriever retriever = new OneStepRetriever(Constants.QUERY_FILE_TEST, corpusFile);
