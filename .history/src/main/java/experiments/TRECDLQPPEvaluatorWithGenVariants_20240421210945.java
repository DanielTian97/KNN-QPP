...
public class TRECDLQPPEvaluatorWithGenVariants {
    ...
    static double trainAndTest(
        ...
        //here ..... 0421
        if(Constants.SAME_RETRIEVER) {
            AllRetrievedResults retRcds = new AllRetrievedResults(rqResFile); //rqResFile should be a constant
        }
    ...
    public static void main(String[] args) {
        ...
        if(Constants.SAME_RETRIEVER) {
            if(retrieverName.equals("mt5")){
                rqResFile = String.format("QV_bm25_%s_%s.res", retrieverName, args[4]);
            }
        }

        switch(args[4]){ // change the rqResFile according to retriever+qv_type
            case "rlm":
