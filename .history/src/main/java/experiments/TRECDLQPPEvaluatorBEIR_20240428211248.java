...
public class TRECDLQPPEvaluatorBEIR {
    ...
    public static void main(String[] args) {
        ...
        String resFileForTrain = args[0];
        String resFileForTest = args[1];
        ...
        if (resFileForTrain.indexOf("mt5") != -1) {
            // mt5
            retrieverName = "mt5";
        } else if (resFileForTrain.indexOf("BM25+BERT") != -1){
            // bert
            retrieverName = "bert";
        }
        ...
        try {
            OneStepRetriever retrieverTrain = new OneStepRetriever(Constants.QUERY_FILE_TEST, Constants.MSMARCO_INDEX);
            OneStepRetriever retrieverTest = new OneStepRetriever(Constants.QUERY_FILE_TEST, corpusIndex);

            Settings.init(retrieverTrain.getSearcher()); // it need be changed when do test/train

            TauAndSARE kendalsOnTest = trainAndTest(basePredictorName, retriever_train, targetMetric,
                    QUERY_FILE_MSMARCO, QRELS_FILE_MSMARCO,
                    QUERY_FILE_BEIR, QRELS_FILE_BEIR,
                    retrieverTrain, retrieverTest,
                    resFileForTrain, resFileForTest, Constants.QPP_COREL_MAX_VARIANTS, 
                    useRBO, extendQV,
                    qvResults);
